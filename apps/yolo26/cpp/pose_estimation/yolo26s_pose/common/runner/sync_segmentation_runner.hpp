/**
 * @file segmentation_runner.hpp
 * @brief Synchronous instance segmentation runner using factory pattern (Legacy API aligned)
 *
 * Provides a generic runner that accepts any IInstanceSegmentationFactory implementation.
 */

#ifndef SEGMENTATION_RUNNER_HPP
#define SEGMENTATION_RUNNER_HPP
...
/**
 * @brief Generic synchronous runner for instance segmentation models
 * @tparam FactoryT The factory type (must derive from IInstanceSegmentationFactory)
 */
template <typename FactoryT>
class SyncInstanceSegRunner {
    bool verbose_ = false;

public:
    explicit SyncInstanceSegRunner(std::unique_ptr<FactoryT> factory)
...
private:
    ...
    /** Print instance segmentation results to stdout for pipeline parsing. */
    static void printInstanceSegResults(const std::vector<InstanceSegmentationResult>& results,
                                        bool verbose = false) {
        if (!verbose) return;
        for (const auto& seg : results) {
            if (seg.box.size() < 4) continue;
            std::string iname = seg.class_name.empty() ? std::to_string(seg.class_id) : seg.class_name;
            for (auto& ch : iname) if (ch == ' ') ch = '_';
            std::cout << "[ISEG] " << iname
                      << " " << std::fixed << std::setprecision(4) << seg.confidence
                      << " " << seg.box[0] << " " << seg.box[1] << " " << seg.box[2] << " " << seg.box[3]
                      << std::endl;
        }
    }
    ...
    bool processSingleFrame(
        ...
        IPostprocessor<InstanceSegmentationResult>& postprocessor,
        IVisualizer<InstanceSegmentationResult>& visualizer,
        SyncProfilingMetrics& metrics,
        ...
        // Print instance segmentation results for pipeline parsing
        printInstanceSegResults(results, verbose_);


        // --- Numerical verification dump (DXAPP_VERIFY=1) ---
        verify::dumpVerifyJson(results, model_path_,
            "instance_segmentation", display_image.rows, display_image.cols);
    ...
    void processImageFrames(
        ...
        IPreprocessor& preprocessor, IPostprocessor<InstanceSegmentationResult>& postprocessor,
        IVisualizer<InstanceSegmentationResult>& visualizer, SyncProfilingMetrics& metrics,
