/**
 * @file async_segmentation_runner.hpp
 * @brief Asynchronous instance segmentation runner using factory pattern
 *
 * Provides a generic async runner that accepts any IInstanceSegmentationFactory implementation.
 */

#ifndef ASYNC_SEGMENTATION_RUNNER_HPP
#define ASYNC_SEGMENTATION_RUNNER_HPP
...
struct AsyncInstanceSegDisplayArgs {
    ...
    AsyncInstanceSegDisplayArgs(AsyncInstanceSegDisplayArgs&&) noexcept = default;
    AsyncInstanceSegDisplayArgs& operator=(AsyncInstanceSegDisplayArgs&&) noexcept = default;
};

template <typename FactoryT>
class AsyncInstanceSegRunner {
    bool verbose_ = false;

public:
    explicit AsyncInstanceSegRunner(std::unique_ptr<FactoryT> factory)
    ...
    int run(int argc, char* argv[]) {
        ...
        ie.RegisterCallback([this, postprocessor](dxrt::TensorPtrs& outputs, void* user_data) -> int {  // NOSONAR(cpp:S5008)
            ...
            std::vector<InstanceSegmentationResult> detections;
            ...
            if (verbose_) {
                for (const auto& seg : detections) {
                    if (seg.box.size() >= 4) {
                        std::string iname = dxapp::sanitize_name(seg.class_name.empty() ? std::to_string(seg.class_id) : seg.class_name);
                        std::cout << "[ISEG] " << iname
            ...
            AsyncInstanceSegDisplayArgs display_args;
            display_args.original_frame = std::make_shared<cv::Mat>(ud->display_frame);
            display_args.detections = std::make_shared<std::vector<InstanceSegmentationResult>>(std::move(detections));
...
#endif  // ASYNC_SEGMENTATION_RUNNER_HPP
