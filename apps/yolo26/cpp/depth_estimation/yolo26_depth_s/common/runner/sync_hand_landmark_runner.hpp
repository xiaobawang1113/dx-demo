/**
 * @file hand_landmark_runner.hpp
 * @brief Synchronous hand landmark runner using factory pattern (Legacy API aligned)
 *
 * Provides a generic runner that accepts any IHandLandmarkFactory implementation.
 */

#ifndef HAND_LANDMARK_RUNNER_HPP
#define HAND_LANDMARK_RUNNER_HPP
...
/**
 * @brief Generic synchronous runner for hand landmark models
 * @tparam FactoryT The factory type (must derive from IHandLandmarkFactory)
 */
template <typename FactoryT>
class SyncHandLandmarkRunner {
    bool verbose_ = false;

public:
    explicit SyncHandLandmarkRunner(std::unique_ptr<FactoryT> factory)
...
private:
    ...
    CommandLineArgs parseCommandLine(int argc, char* argv[]) {
        ...
        options.add_options()
            ("m, model_path", "hand landmark model file (.dxnn, required)",
    ...
    bool processSingleFrame(
        ...
        IPostprocessor<HandLandmarkResult>& postprocessor,
        IVisualizer<HandLandmarkResult>& visualizer,
        SyncProfilingMetrics& metrics,
        ...
        // Print hand landmark results for pipeline parsing
        if (!results.empty() && verbose_) {
            std::cout << "[HAND]";
            for (const auto& h : results) {
                std::cout << " " << (h.handedness.empty() ? "unknown" : h.handedness)
                          << " " << std::fixed << std::setprecision(2) << h.confidence;
            }
            std::cout << std::endl;
        }
        ...
        verify::dumpVerifyJson(results, model_path_,
            "hand_landmark", display_image.rows, display_image.cols);
    ...
    void processImageFrames(
        ...
        IPreprocessor& preprocessor, IPostprocessor<HandLandmarkResult>& postprocessor,
        IVisualizer<HandLandmarkResult>& visualizer, SyncProfilingMetrics& metrics,
...
#endif  // HAND_LANDMARK_RUNNER_HPP
