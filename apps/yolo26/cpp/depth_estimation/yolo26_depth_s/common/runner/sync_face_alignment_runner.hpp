/**
 * @file face_alignment_runner.hpp
 * @brief Synchronous face alignment runner using factory pattern (Legacy API aligned)
 *
 * Provides a generic runner that accepts any IFaceAlignmentFactory implementation.
 */

#ifndef FACE_ALIGNMENT_RUNNER_HPP
#define FACE_ALIGNMENT_RUNNER_HPP
...
/**
 * @brief Generic synchronous runner for face alignment models
 * @tparam FactoryT The factory type (must derive from IFaceAlignmentFactory)
 */
template <typename FactoryT>
class SyncFaceAlignmentRunner {
    bool verbose_ = false;

public:
    explicit SyncFaceAlignmentRunner(std::unique_ptr<FactoryT> factory)
...
private:
    ...
    CommandLineArgs parseCommandLine(int argc, char* argv[]) {
        ...
        options.add_options()
            ("m, model_path", "face alignment model file (.dxnn, required)",
    ...
    bool processSingleFrame(
        ...
        IPostprocessor<FaceAlignmentResult>& postprocessor,
        IVisualizer<FaceAlignmentResult>& visualizer,
        SyncProfilingMetrics& metrics,
        ...
        // Print face alignment results for pipeline parsing (verbose only)
        if (verbose_) {
            for (const auto& fa : results) {
                if (fa.pose.size() >= 3) {
                    std::cout << "[ALIGN] " << std::fixed << std::setprecision(1)
                              << fa.pose[0] << " " << fa.pose[1] << " " << fa.pose[2] << std::endl;
                }
            }
        }
        ...
        verify::dumpVerifyJson(results, model_path_,
            "face_alignment", display_image.rows, display_image.cols);
    ...
    void processImageFrames(
        ...
        IPreprocessor& preprocessor, IPostprocessor<FaceAlignmentResult>& postprocessor,
        IVisualizer<FaceAlignmentResult>& visualizer, SyncProfilingMetrics& metrics,
...
#endif  // FACE_ALIGNMENT_RUNNER_HPP
