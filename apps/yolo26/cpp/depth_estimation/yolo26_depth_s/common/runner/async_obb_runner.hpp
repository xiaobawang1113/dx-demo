/**
 * @file async_obb_runner.hpp
 * @brief Asynchronous OBB (Oriented Bounding Box) runner using factory pattern
 *
 * Provides a generic async runner that accepts any IOBBFactory implementation.
 */

#ifndef ASYNC_OBB_RUNNER_HPP
#define ASYNC_OBB_RUNNER_HPP
...
struct AsyncOBBDisplayArgs {
    std::shared_ptr<std::vector<OBBResult>> detections;
    ...
    AsyncOBBDisplayArgs(AsyncOBBDisplayArgs&&) noexcept = default;
    AsyncOBBDisplayArgs& operator=(AsyncOBBDisplayArgs&&) noexcept = default;
};

template <typename FactoryT>
class AsyncOBBRunner {
    bool verbose_ = false;

public:
    explicit AsyncOBBRunner(std::unique_ptr<FactoryT> factory)
    ...
    int run(int argc, char* argv[]) {
        ...
        ie.RegisterCallback([this, postprocessor](dxrt::TensorPtrs& outputs, void* user_data) -> int {  // NOSONAR(cpp:S5008)
            ...
            AsyncOBBDisplayArgs display_args;
            display_args.original_frame = std::make_shared<cv::Mat>(ud->display_frame);
            display_args.detections = std::make_shared<std::vector<OBBResult>>(std::move(detections));
            ...
            verify::dumpVerifyJson(*display_args.detections, model_path_,
                "obb_detection", display_args.original_frame->rows, display_args.original_frame->cols);
...
private:
    ...
    SafeQueue<AsyncOBBDisplayArgs> display_queue_;
    SafeQueue<cv::Mat> rendered_queue_;  // Rendered frames for main-thread display
    AsyncProfilingMetrics metrics_;

    CommandLineArgs parseCommandLine(int argc, char* argv[]) {
        CommandLineArgs args;
        std::string app_name = factory_->getModelName() + " OBB Post-Processing Async Example";
    ...
    void displayThread(IVisualizer<OBBResult>& visualizer, bool no_display,
