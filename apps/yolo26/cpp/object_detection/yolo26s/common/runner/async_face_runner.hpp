/**
 * @file async_face_runner.hpp
 * @brief Asynchronous face detection runner using factory pattern
 *
 * Provides a generic async runner that accepts any IFaceDetectionFactory implementation.
 */

#ifndef ASYNC_FACE_RUNNER_HPP
#define ASYNC_FACE_RUNNER_HPP
...
// Display arguments for async face processing
struct AsyncFaceDisplayArgs {
    std::shared_ptr<std::vector<FaceDetectionResult>> detections;
    ...
    AsyncFaceDisplayArgs() = default;
    AsyncFaceDisplayArgs(const AsyncFaceDisplayArgs&) = default;
    AsyncFaceDisplayArgs& operator=(const AsyncFaceDisplayArgs&) = default;
    AsyncFaceDisplayArgs(AsyncFaceDisplayArgs&&) noexcept = default;
    AsyncFaceDisplayArgs& operator=(AsyncFaceDisplayArgs&&) noexcept = default;
};

/**
 * @brief Generic asynchronous runner for face detection models
 * @tparam FactoryT The factory type (must derive from IFaceDetectionFactory)
 */
template <typename FactoryT>
class AsyncFaceRunner {
    bool verbose_ = false;

public:
    explicit AsyncFaceRunner(std::unique_ptr<FactoryT> factory)
    ...
    int run(int argc, char* argv[]) {
        ...
        if (is_image) {
            ...
            for (int i = 0; i < loopTest && running_ && !g_interrupted(); ++i) {
                ...
                std::string save_path;
                if (!run_dir.empty()) {
                    save_path = dxapp::buildPerImageSavePath(run_dir, factory_->getModelName() + "_async", currentImagePath, i);
                }
            ...
            for (int loop_idx = 0; loop_idx < loopTest && running_ && !g_interrupted(); ++loop_idx) {
                ...
                if (!video.isOpened()) {
                    std::cerr << "[ERROR] Failed to reopen video: " << args.videoFile << std::endl;
                    break;
                }
            }
        }
...
private:
    ...
    int onAsyncInferenceComplete(
        ...
        IPostprocessor<FaceDetectionResult>& postprocessor) {
        ...
        try {
            detections = postprocessor.process(outputs, ud_ptr->ctx);
        ...
        printFaceDetectionResults(detections, verbose_);
        completeInflightMetrics();

        AsyncFaceDisplayArgs display_args;
        display_args.original_frame = std::make_shared<cv::Mat>(ud_ptr->display_frame);
        display_args.detections = std::make_shared<std::vector<FaceDetectionResult>>(std::move(detections));
        ...
        verify::dumpVerifyJson(*display_args.detections, model_path_,
            "face_detection", display_args.original_frame->rows, display_args.original_frame->cols);
    ...
    /** Print face detection results for pipeline parsing. */
    static void printFaceDetectionResults(const std::vector<FaceDetectionResult>& detections, bool verbose = false) {
        if (!verbose || detections.empty()) return;
        std::cout << "[FACE] " << detections.size();
        for (const auto& f : detections) {
            std::cout << " " << std::fixed << std::setprecision(4) << f.confidence;
        }
        std::cout << std::endl;
    }
    ...
    /** Preprocess a single frame and submit it for async inference. */
    void processFrameAsync(
        const cv::Mat& frame, cv::Mat& display_image,
        IPreprocessor& preprocessor,
        std::vector<std::vector<uint8_t>>& input_buffers,
        int& buffer_index, dxrt::InferenceEngine& ie,
        int& last_job_id, int& processCount) {
        auto t_pre_start = std::chrono::high_resolution_clock::now();
    ...
    void displayThread(IVisualizer<FaceDetectionResult>& visualizer, bool no_display,
                       bool save_on, cv::VideoWriter& writer) {
        while (running_) {
            AsyncFaceDisplayArgs args;
