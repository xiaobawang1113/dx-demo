/**
 * @file async_hand_landmark_runner.hpp
 * @brief Asynchronous hand landmark runner using factory pattern
 *
 * Provides a generic async runner that accepts any IHandLandmarkFactory implementation.
 */

#ifndef ASYNC_HAND_LANDMARK_RUNNER_HPP
#define ASYNC_HAND_LANDMARK_RUNNER_HPP
...
struct AsyncPoseDisplayArgs {
    std::shared_ptr<std::vector<HandLandmarkResult>> detections;
...
class AsyncHandLandmarkRunner {
    bool verbose_ = false;

public:
    explicit AsyncHandLandmarkRunner(std::unique_ptr<FactoryT> factory)
    ...
    int run(int argc, char* argv[]) {
        ...
        ie.RegisterCallback([this, postprocessor](dxrt::TensorPtrs& outputs, void* user_data) -> int {  // NOSONAR(cpp:S5008)
            ...
            std::vector<HandLandmarkResult> detections;
            ...
            display_args.detections = std::make_shared<std::vector<HandLandmarkResult>>(std::move(detections));
            ...
            verify::dumpVerifyJson(*display_args.detections, model_path_,
                "hand_landmark", display_args.original_frame->rows, display_args.original_frame->cols);
...
private:
    ...
    void displayThread(IVisualizer<HandLandmarkResult>& visualizer, bool no_display,
...
#endif  // ASYNC_HAND_LANDMARK_RUNNER_HPP
