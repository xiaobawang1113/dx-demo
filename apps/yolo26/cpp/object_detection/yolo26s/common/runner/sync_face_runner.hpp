/**
 * @file face_runner.hpp
 * @brief Synchronous face detection runner using factory pattern (Legacy API aligned)
 *
 * Provides a generic runner that accepts any IFaceDetectionFactory implementation.
 */

#ifndef FACE_RUNNER_HPP
#define FACE_RUNNER_HPP
...
/**
 * @brief Generic synchronous runner for face detection models
 * @tparam FactoryT The factory type (must derive from IFaceDetectionFactory)
 */
template <typename FactoryT>
class SyncFaceRunner {
    bool verbose_ = false;

public:
    explicit SyncFaceRunner(std::unique_ptr<FactoryT> factory)
...
private:
    ...
    CommandLineArgs parseCommandLine(int argc, char* argv[]) {
        ...
        options.add_options()
            ("m, model_path", "face detection model file (.dxnn, required)",
    ...
    bool processSingleFrame(
        ...
        IVisualizer<FaceDetectionResult>& visualizer,
        SyncProfilingMetrics& metrics,
        ...
        try {
            ...
            { const char* _sv=std::getenv("DXAPP_SAVE_IMAGE"); if(_sv&&*_sv)cv::imwrite(_sv,display_image); }
            metrics.sum_read += t_read;
            metrics.sum_preprocess += t_preprocess;
            metrics.sum_inference += t_inference;
            metrics.infer_completed++;
        ...
        verify::dumpVerifyJson(detections, model_path_,
            "face_detection", display_image.rows, display_image.cols);

        auto render_start = std::chrono::high_resolution_clock::now();
        cv::Mat result_frame = display_image.clone();
        result_frame = visualizer.draw(result_frame, detections, ctx);
    ...
    void processImageFrames(
        ...
        IPreprocessor& preprocessor, IPostprocessor<FaceDetectionResult>& postprocessor,
        IVisualizer<FaceDetectionResult>& visualizer, SyncProfilingMetrics& metrics,
