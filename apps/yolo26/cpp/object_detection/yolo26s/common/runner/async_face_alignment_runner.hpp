/**
 * @file async_face_alignment_runner.hpp
 * @brief Asynchronous face alignment runner using factory pattern
 *
 * Provides a generic async runner that accepts any IFaceAlignmentFactory implementation.
 */

#ifndef ASYNC_FACE_ALIGNMENT_RUNNER_HPP
#define ASYNC_FACE_ALIGNMENT_RUNNER_HPP
...
struct AsyncPoseDisplayArgs {
    std::shared_ptr<std::vector<FaceAlignmentResult>> detections;
...
class AsyncFaceAlignmentRunner {
    bool verbose_ = false;

public:
    explicit AsyncFaceAlignmentRunner(std::unique_ptr<FactoryT> factory)
    ...
    int run(int argc, char* argv[]) {
        ...
        ie.RegisterCallback([this, postprocessor](dxrt::TensorPtrs& outputs, void* user_data) -> int {  // NOSONAR(cpp:S5008)
            ...
            std::vector<FaceAlignmentResult> detections;
            try { detections = postprocessor->process(outputs, ud_ptr->ctx); }
            ...
            display_args.detections = std::make_shared<std::vector<FaceAlignmentResult>>(std::move(detections));
            ...
            verify::dumpVerifyJson(*display_args.detections, model_path_,
                "face_alignment", display_args.original_frame->rows, display_args.original_frame->cols);
...
private:
    ...
    void displayThread(IVisualizer<FaceAlignmentResult>& visualizer, bool no_display,
...
#endif  // ASYNC_FACE_ALIGNMENT_RUNNER_HPP
