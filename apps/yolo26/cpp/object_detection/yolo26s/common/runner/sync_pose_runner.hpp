/**
 * @file pose_runner.hpp
 * @brief Synchronous pose estimation runner using factory pattern (Legacy API aligned)
 *
 * Provides a generic runner that accepts any IPoseFactory implementation.
 */

#ifndef POSE_RUNNER_HPP
#define POSE_RUNNER_HPP
...
/**
 * @brief Generic synchronous runner for pose estimation models
 * @tparam FactoryT The factory type (must derive from IPoseFactory)
 */
template <typename FactoryT>
class SyncPoseRunner {
    bool verbose_ = false;

public:
    explicit SyncPoseRunner(std::unique_ptr<FactoryT> factory)
    ...
    int run(int argc, char* argv[]) {
        ...
        PipelineContext pipeline{ie, *preprocessor, *postprocessor, *visualizer, metrics, writer, args.no_display, args.saveMode};

        if (is_image) {
            processImageFrames(imageFiles, loopTest, display_image,
                               pipeline, processCount,
...
private:
    ...
    /** Bundles commonly-shared pipeline objects to reduce parameter count. */
    struct PipelineContext {
        dxrt::InferenceEngine& ie;
        IPreprocessor& preprocessor;
        IPostprocessor<PoseResult>& postprocessor;
        IVisualizer<PoseResult>& visualizer;
        SyncProfilingMetrics& metrics;
        cv::VideoWriter& writer;
        bool no_display;
        bool saveMode;
    ...
    CommandLineArgs parseCommandLine(int argc, char* argv[]) {
        ...
        options.add_options()
            ("m, model_path", "pose estimation model file (.dxnn, required)",
    ...
    bool processSingleFrame(
        ...
        PipelineContext& ctx, double t_read,
        int frameIdx = 0,
        const std::string& dumpTensorsDir = "",
        bool dumpPerFrameDir = false) {

        auto& ie = ctx.ie;
        auto& preprocessor = ctx.preprocessor;
        auto& postprocessor = ctx.postprocessor;
        auto& visualizer = ctx.visualizer;
        auto& metrics = ctx.metrics;
        auto& writer = ctx.writer;
        bool no_display = ctx.no_display;
        bool saveMode = ctx.saveMode;
        ...
        PreprocessContext preprocess_ctx;
        cv::Mat preprocessed;
        preprocessor.process(display_image, preprocessed, preprocess_ctx);
        ...
        try {
            results = postprocessor.process(outputs, preprocess_ctx);
            ...
            // Save original image so GUI still shows something
    ...
    void processImageFrames(
        ...
        cv::Mat& display_image, PipelineContext& ctx,
        int& processCount,
    ...
    void processVideoFrames(
        ...
        PipelineContext& ctx, int& processCount,
        ...
        for (int loop_idx = 0; loop_idx < loopTest && !g_interrupted().load(); ++loop_idx) {
            ...
            while (!g_interrupted().load() && readFrame(frame)) {
                if (!processSingleFrame(frame, display_image, ctx, t_read,
