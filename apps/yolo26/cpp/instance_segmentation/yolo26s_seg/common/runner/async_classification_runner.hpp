/**
 * @file async_classification_runner.hpp
 * @brief Asynchronous classification runner using factory pattern
 *
 * Provides a generic async runner that accepts any IClassificationFactory implementation.
 * Note: Classification factories do NOT take is_ort_configured parameter.
 */

#ifndef ASYNC_CLASSIFICATION_RUNNER_HPP
#define ASYNC_CLASSIFICATION_RUNNER_HPP
...
struct AsyncClassificationDisplayArgs {
    std::shared_ptr<std::vector<ClassificationResult>> results;
    ...
    AsyncClassificationDisplayArgs(AsyncClassificationDisplayArgs&&) noexcept = default;
    AsyncClassificationDisplayArgs& operator=(AsyncClassificationDisplayArgs&&) noexcept = default;
};

template <typename FactoryT>
class AsyncClassificationRunner {
    bool verbose_ = false;

public:
    explicit AsyncClassificationRunner(std::unique_ptr<FactoryT> factory)
    ...
    int run(int argc, char* argv[]) {
        ...
        // Classification: no is_ort_configured parameter
        ...
        ie.RegisterCallback([this, postprocessor](dxrt::TensorPtrs& outputs, void* user_data) -> int {  // NOSONAR(cpp:S5008)
            ...
            display_args.results = std::make_shared<std::vector<ClassificationResult>>(std::move(results));
...
private:
    ...
    SafeQueue<AsyncClassificationDisplayArgs> display_queue_;
...
#endif  // ASYNC_CLASSIFICATION_RUNNER_HPP
