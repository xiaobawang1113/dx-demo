/**
 * @file sync_embedding_runner.hpp
 * @brief Synchronous embedding/feature extraction runner using factory pattern
 *
 * Provides a generic runner that accepts any IEmbeddingFactory implementation.
 */

#ifndef EMBEDDING_RUNNER_HPP
#define EMBEDDING_RUNNER_HPP
...
private:
    ...
    CommandLineArgs parseCommandLine(int argc, char* argv[]) {
        ...
        std::string app_name = factory_->getModelName() + " Embedding Sync Example";
        cxxopts::Options options(app_name, app_name + " application usage ");
        options.add_options()
            ("m, model_path", "embedding model file (.dxnn, required)",
    ...
    bool processSingleFrame(
        ...
        IPostprocessor<EmbeddingResult>& postprocessor,
        IVisualizer<EmbeddingResult>& visualizer,
        SyncProfilingMetrics& metrics,
        ...
        verify::dumpVerifyJson(results, model_path_,
            "embedding", display_image.rows, display_image.cols);
    ...
    void processImageFrames(
        ...
        IPreprocessor& preprocessor, IPostprocessor<EmbeddingResult>& postprocessor,
        IVisualizer<EmbeddingResult>& visualizer, SyncProfilingMetrics& metrics,
