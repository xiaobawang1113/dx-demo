...
private:
    ...
    bool processSingleFrame(
        ...
        if (input_channels <= 1) {
            ...
            // Use original image dimensions padded up to tile boundaries
            // to preserve full resolution (no downscaling)
            int lr_w = ((input_frame.cols + tile_w - 1) / tile_w) * tile_w;
            int lr_h = ((input_frame.rows + tile_h - 1) / tile_h) * tile_h;
