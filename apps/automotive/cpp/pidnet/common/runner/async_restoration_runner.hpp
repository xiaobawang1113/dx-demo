...
private:
    ...
    void processFrameSR(const cv::Mat& frame, dxrt::InferenceEngine& ie, int& processCount,
        ...
        auto t_pre_start = std::chrono::high_resolution_clock::now();
        // Use original frame dimensions padded up to tile boundaries
        // to preserve full resolution (no downscaling)
        int lr_w = ((frame.cols + tile_w_ - 1) / tile_w_) * tile_w_;
        int lr_h = ((frame.rows + tile_h_ - 1) / tile_h_) * tile_h_;

        cv::Mat lr_bgr;
        cv::resize(frame, lr_bgr, cv::Size(lr_w, lr_h));
        cv::Mat lr_gray;
        cv::cvtColor(lr_bgr, lr_gray, cv::COLOR_BGR2GRAY);
        auto t_pre_end = std::chrono::high_resolution_clock::now();

        int out_w = lr_w * scale_x_;
        ...
        auto t_post_start = std::chrono::high_resolution_clock::now();
        cv::Mat canvas = buildSRCanvas(lr_bgr, sr_y, out_w, out_h, lr_w, lr_h, tiles_done);
        auto t_post_end = std::chrono::high_resolution_clock::now();
