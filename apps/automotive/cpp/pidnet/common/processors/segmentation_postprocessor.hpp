...
// ============================================================================
// PIDNet Semantic Segmentation Postprocessor
// ============================================================================
class PidNetPostprocessor : public IPostprocessor<SegmentationResult> {
public:
    PidNetPostprocessor(int input_width = 2048, int input_height = 1024,
                        double argmax_scale = 0.0)
        : input_width_(input_width), input_height_(input_height),
          argmax_scale_(argmax_scale) {}
    ...
    std::vector<SegmentationResult> process(const dxrt::TensorPtrs& outputs,
        ...
        if (!parseShape(tensor->shape(), info)) {
            std::cerr << "[DXAPP] [ER] PIDNet postprocess expects 2D/3D/4D class logits or class map, got shape: "
                      << shapeToString(tensor->shape()) << std::endl;
            return results;
        }

        cv::Mat low_mask;
        if (info.is_class_map) {
            if (!fillClassMap(tensor->data(), tensor->elem_size(), tensor->type(), info, low_mask)) {
                std::cerr << "[DXAPP] [ER] PIDNet postprocess expects integer or float class map, got elem_size: "
                          << tensor->elem_size() << std::endl;
                return results;
            }
        } else {
            if (tensor->elem_size() != 4) {
                std::cerr << "[DXAPP] [ER] PIDNet postprocess expects float logits, got elem_size: "
                          << tensor->elem_size() << std::endl;
                return results;
            }
            int target_w = ctx.original_width > 0 ? ctx.original_width : input_width_;
            int target_h = ctx.original_height > 0 ? ctx.original_height : input_height_;
            if (argmax_scale_ > 0.0) {
                int scaled_w = scaledArgmaxSize(info.W, target_w, argmax_scale_);
                int scaled_h = scaledArgmaxSize(info.H, target_h, argmax_scale_);
                fillArgmaxMaskScaled(static_cast<const float*>(tensor->data()), info,
                                     scaled_w, scaled_h, low_mask);
            } else {
                fillArgmaxMask(static_cast<const float*>(tensor->data()), info, low_mask);
            }
        }
        ...
        int target_h = ctx.original_height > 0 ? ctx.original_height : input_height_;
        cv::Mat resized_mask;
        resizeMaskNearest(low_mask, resized_mask, target_w, target_h);

        SegmentationResult seg;
        seg.width = resized_mask.cols;
        seg.height = resized_mask.rows;
        seg.mask.resize(seg.width * seg.height);

        std::set<int> unique_classes;
        for (int y = 0; y < resized_mask.rows; ++y) {
            const int* src = resized_mask.ptr<int>(y);
            for (int x = 0; x < resized_mask.cols; ++x) {
                int cls = src[x];
                seg.mask[y * resized_mask.cols + x] = cls;
                unique_classes.insert(cls);
            }
        }
        seg.class_ids.assign(unique_classes.begin(), unique_classes.end());
    ...
    std::string getModelName() const override { return "PIDNet"; }
...
private:
    ...
    static bool parseShape(const std::vector<int64_t>& shape, TensorShapeInfo& info) {
        if (shape.size() == 4) {
            if (shape[1] <= shape[3]) {
                info.C = static_cast<int>(shape[1]);
                info.H = static_cast<int>(shape[2]);
                info.W = static_cast<int>(shape[3]);
                info.is_nhwc = false;
            } else {
                info.H = static_cast<int>(shape[1]);
                info.W = static_cast<int>(shape[2]);
                info.C = static_cast<int>(shape[3]);
                info.is_nhwc = true;
            }
            info.is_class_map = info.C == 1;
            return info.H > 0 && info.W > 0 && info.C > 0;
        }
        if (shape.size() == 3) {
            if (shape[0] <= shape[1] && shape[0] <= shape[2]) {
                info.C = static_cast<int>(shape[0]);
                info.H = static_cast<int>(shape[1]);
                info.W = static_cast<int>(shape[2]);
                info.is_nhwc = false;
            } else {
                info.H = static_cast<int>(shape[0]);
                info.W = static_cast<int>(shape[1]);
                info.C = static_cast<int>(shape[2]);
                info.is_nhwc = true;
            }
            info.is_class_map = info.C == 1;
            return info.H > 0 && info.W > 0 && info.C > 0;
        ...
        if (shape.size() == 2) {
            ...
            info.H = static_cast<int>(shape[0]);
            info.W = static_cast<int>(shape[1]);
            info.is_class_map = true;
            return info.H > 0 && info.W > 0;
        }
        return false;
    }

    static void fillArgmaxMask(const float* data, const TensorShapeInfo& info,
                               cv::Mat& low_mask) {
        low_mask = cv::Mat(info.H, info.W, CV_32SC1);
        for (int y = 0; y < info.H; ++y) {
            int* dst = low_mask.ptr<int>(y);
            for (int x = 0; x < info.W; ++x) {
                float max_val = -1e30f;
                int max_cls = 0;
                for (int c = 0; c < info.C; ++c) {
                    float val = info.is_nhwc
                        ? data[(y * info.W + x) * info.C + c]
                        : data[c * info.H * info.W + y * info.W + x];
                    if (val > max_val) {
                        max_val = val;
                        max_cls = c;
                    }
    ...
    static int scaledArgmaxSize(int low_size, int target_size, double scale) {
        int scaled = static_cast<int>(std::round(target_size * scale));
        scaled = std::max(low_size, scaled);
        scaled = std::min(target_size, scaled);
        return std::max(1, scaled);
    }

    static float bilinearAt(const float* data, const TensorShapeInfo& info,
                            int c, float src_y, float src_x) {
        src_y = std::max(0.0f, std::min(src_y, static_cast<float>(info.H - 1)));
        src_x = std::max(0.0f, std::min(src_x, static_cast<float>(info.W - 1)));
        int y0 = static_cast<int>(std::floor(src_y));
        int x0 = static_cast<int>(std::floor(src_x));
        int y1 = std::min(y0 + 1, info.H - 1);
        int x1 = std::min(x0 + 1, info.W - 1);
        float wy = src_y - y0;
        float wx = src_x - x0;
        auto value = [&](int y, int x) {
            return info.is_nhwc
                ? data[(y * info.W + x) * info.C + c]
                : data[c * info.H * info.W + y * info.W + x];
        };
        float v00 = value(y0, x0);
        float v01 = value(y0, x1);
        float v10 = value(y1, x0);
        float v11 = value(y1, x1);
        float top = v00 + (v01 - v00) * wx;
        float bottom = v10 + (v11 - v10) * wx;
        return top + (bottom - top) * wy;
    }

    static void fillArgmaxMaskScaled(const float* data, const TensorShapeInfo& info,
                                     int out_w, int out_h, cv::Mat& mask) {
        if (out_w == info.W && out_h == info.H) {
            fillArgmaxMask(data, info, mask);
            return;
        }
        mask = cv::Mat(out_h, out_w, CV_32SC1);
        float scale_x = (out_w > 1) ? static_cast<float>(info.W - 1) / static_cast<float>(out_w - 1) : 0.0f;
        float scale_y = (out_h > 1) ? static_cast<float>(info.H - 1) / static_cast<float>(out_h - 1) : 0.0f;
        for (int y = 0; y < out_h; ++y) {
            int* dst = mask.ptr<int>(y);
            ...
            for (int x = 0; x < out_w; ++x) {
                ...
                int max_cls = 0;
                for (int c = 0; c < info.C; ++c) {
                    float val = bilinearAt(data, info, c, src_y, src_x);
                    if (val > max_val) {
                        max_val = val;
                        max_cls = c;
                    }
    ...
    static bool fillClassMap(const void* raw, size_t elem_size, dxrt::DataType dtype,
                             const TensorShapeInfo& info, cv::Mat& low_mask) {
        low_mask = cv::Mat(info.H, info.W, CV_32SC1);
        for (int y = 0; y < info.H; ++y) {
            int* dst = low_mask.ptr<int>(y);
            for (int x = 0; x < info.W; ++x) {
                size_t idx = static_cast<size_t>(y) * info.W + x;
                if (dtype == dxrt::DataType::INT64 && elem_size == 8) dst[x] = static_cast<int>(static_cast<const int64_t*>(raw)[idx]);
                else if (dtype == dxrt::DataType::INT32 && elem_size == 4) dst[x] = static_cast<int>(static_cast<const int32_t*>(raw)[idx]);
                else if (dtype == dxrt::DataType::FLOAT && elem_size == 4) dst[x] = static_cast<int>(std::round(static_cast<const float*>(raw)[idx]));
                else if (elem_size == 2) dst[x] = static_cast<int>(static_cast<const int16_t*>(raw)[idx]);
                else if (dtype == dxrt::DataType::UINT8 && elem_size == 1) dst[x] = static_cast<int>(static_cast<const uint8_t*>(raw)[idx]);
                else return false;
            }
        }
        return true;
    ...
    static void resizeMaskNearest(const cv::Mat& low_mask, cv::Mat& resized_mask,
                                  int target_w, int target_h) {
        if (low_mask.cols == target_w && low_mask.rows == target_h) {
            resized_mask = low_mask.clone();
            return;
        }
        cv::Mat float_mask;
        low_mask.convertTo(float_mask, CV_32FC1);
        cv::Mat resized_float;
        cv::resize(float_mask, resized_float, cv::Size(target_w, target_h),
                   0, 0, cv::INTER_NEAREST);
        resized_float.convertTo(resized_mask, CV_32SC1);
    }

    int input_width_;
    int input_height_;
    double argmax_scale_;
};
