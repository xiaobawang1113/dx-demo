...
#### DX-Compiler: ONNX to DXNN (DX-Compiler v2.2.1)
```bash
# Move to dx-compiler path
cd <path of dx-all-suite>/dx-compiler

# Activate dx-compiler python virtual environment
source venv-dx-compiler-local/bin/activate

# Compile using dx-compiler
dxcom \
    -m <path of dx-clip-starter>/onnx/ViT-L-14-quickgelu-dfn2b.onnx \
    -c <path of dx-clip-starter>/onnx/ViT-L-14-quickgelu-dfn2b.json \
    -o <path of dx-clip-starter>/dxnn

# Back to the previous path
cd <path of dx-clip-starter>
```
