...
class DetectionNode(Node):
    ...
    def __call__(self, img):
        ...
        if mapped_res not in self.det_model_map:
            # M1 often loads 640-only; det_router still returns 960 for large sides.
            mapped_res = min(self.det_model_map.keys())
...
class AsyncPipelineOCR:
    ...
    def _submit_detection(self, job: OCRJob):
        ...
        if mapped_res not in self.det_models:
            mapped_res = next(iter(self.det_models))
